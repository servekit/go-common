...
仓库根目录有一份使用指南 `README.md`，面向调用方介绍各模块的正确 API 用法。当仓库本身做了影响调用方的变更时，必须同步更新 README —— 否则使用方按过时文档写代码会踩坑（构造函数签名变了、Config 字段加了默认值、新模块没人知道存在）。

**需要更新 README 的场景：**

- **新增模块或子包**（例如加 `ocr/`）：在 README 的"模块速查"表加一行，并新增对应小节（Config / 构造函数 / 最简示例 / 关键约定）
...
**判断标准：** 使用方读完 README 后能否写出与最新代码一致的调用。如果能，不用动；如果不能，就需要更新。
